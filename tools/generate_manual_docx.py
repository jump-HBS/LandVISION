...
DESCRIPTIONS = {
    ...
    "backend/tests/test_smoke.py": "全链路冒烟测试（16 项）：健康检查、地块 CRUD/期次筛选/锁定/批量删除、SHP 导入端到端（内存 zip）、行政区、POI、三区三线体检（规则矩阵/判定依据/锁定）、项目 CRUD 与范围校验、四大结果持久化、地图标注、驾驶舱统筹、综合分析报告（校验六章节与 12 大类）。",
    ...
    "backend/app/schemas.py": "Pydantic 模型（v2.0）：12 大类用地枚举、标准三线代码与中文标签映射（ZONE_TYPE_LABELS）、地块/POI/控制线（含期次/项目/锁定）、分析项目、规则矩阵、批量操作与地图标注等请求响应模型、分页结构。",
    ...
    "backend/app/routers/parcels.py": "地块路由（v2.0）：分页列表/GeoJSON（均支持 period 期次过滤）/CRUD/SHP 批量导入（关联项目与期次）/批量删除（跳过锁定）/锁定解锁/批量设置期次。",
    "backend/app/routers/pois.py": "兴趣点路由：分页列表/GeoJSON/按类型过滤/新建/删除/批量删除（跳过锁定）。",
    "backend/app/routers/analysis.py": "空间分析路由（模块一~三，v2.0）：转移矩阵（期次 SHP 导入/演示基期/计算，结果持久化）+ 图斑查询、适宜性评价（刚性约束+格网持久化）+ 格网查询、可达性分析（结果持久化）+ 结果查询、盲区∩适宜区设施选址、SHP 范围解析。",
    "backend/app/routers/planning.py": "三区三线体检路由（模块四，v2.0）：标准三线控制线管理（含批量删除/锁定/SHP 导入）、规则矩阵查看与更新（GET/PUT rules）、单地块/任意几何体检（含判定依据）、批量体检（结果持久化）、变化图斑体检（模块一→四联动）、结果查询、台账 CSV 导出。",
    ...
    "backend/app/services/spatial.py": "空间查询与地块 CRUD 服务（v2.0）：分页+bbox 视野查询（含 period 过滤）、GeoJSON 序列化、面积自动计算、编号唯一校验、锁定/批量删除/批量设置期次；双模式分支的样板。",
    "backend/app/services/regions.py": "行政区服务：省/市/县查询、GeoJSON、定位（质心+包围盒）、SHP 导入。",
    "backend/app/services/shp_import.py": "SHP 导入服务（v2.0）：zip 解析（pyshp）→ 坐标系校验（仅 WGS84）→ 字段自动/手动映射 → 地类容错映射到 12 大类 → 批量入库（关联项目与期次，返回 imported/skipped）。",
    "backend/app/services/planning_check.py": "三区三线体检服务（模块四，v2.0 规则矩阵版）：标准三线控制线管理、单地块/任意几何体检（规则矩阵判定 + 判定依据含亩数）、批量占用叠加 review_occupancy（结果持久化 planning_check_results）、变化图斑体检 review_patches（回写图斑冲突标记）、台账 CSV。",
    "backend/app/services/analysis.py": "空间分析服务（模块一~三，v2.0）：transition_matrix 两期叠加求交（结果持久化 land_change_patches）、期次导入、演示基期、suitability_evaluate 格网法多因子加权叠加（三区三线刚性约束强制不适宜 + 格网持久化）、accessibility_analyze 生活圈覆盖判定（结果持久化）、facility_sites 盲区∩适宜区选址；DEMO/POSTGIS 共用同一 shapely 实现。",
    ...
    "backend/app/services/report_gen.py": "驾驶舱统筹汇总 + 综合分析报告服务（v2.0，共用数据源）：collect_dashboard 优先从持久化结果表读取（land_change_patches/suitability_grids/planning_check_results/accessibility_results），无结果时实时计算兜底；组装项目概况、流程进度、问题清单与规划建议；to_markdown 输出 项目概况→现状评价→问题识别→原因分析→规划建议→附录 六章节报告。",
    ...
    "frontend/src/stores/ui.js": "Pinia UI 状态（v2.0）：深浅主题、图层可见性、行政区筛选、框选结果、分析项目上下文（currentProject/currentProjectId）、模块联动参数（linkedPatches/linkedScope）、顶栏通知。",
    "frontend/src/api/index.js": "接口封装汇总：按后端 10 组路由分组导出全部 API 函数（含 multipart 上传与 blob 下载），页面不直接写 URL。",
    "frontend/src/utils/request.js": "Axios 实例封装：baseURL=/api、请求/响应拦截器、统一错误提示（适配后端 {code,message,detail}）、超时设置。",
    "frontend/src/utils/colors.js": "配色规范（v2.0）：12 大类国标色、POI 类型色、变化图斑色；三区三线标准术语与制图样式（ZONE_TYPE_LABELS/ZONE_TYPE_COLORS/ZONE_TYPE_LINE_STYLES：红线 #E53935 实线 3px、基本农田 #FFB300 实线 2.5px、开发边界 #1E88E5 虚线 2px）；体检结论配色；图例元数据。",
    ...
    "frontend/src/components/MapView.vue": "核心地图组件（v2.0）：OpenFreeMap 底图+卫星影像、图层管理（地块/POI/三区三线规范线型/变化图斑/适宜性格网/可达性覆盖/行政区边界）、框选圈选多边形选择统计、距离面积测量、悬停高亮、点击弹窗、批量选择模式（点击地块/POI/控制线进入选中集，抛 batch-selection）、保存绘制（测量/勾绘入库 map_features，抛 save-drawing）、地图视野持久化、图例与图层面板、对外暴露 flyTo/fitBounds/clearBatchSelection/clearDrawing/getMap。",
    ...
    "frontend/src/views/DashboardView.vue": "数据驾驶舱（v2.0 项目工作台）：当前项目信息（名称/基期末期年份/范围）与流程进度条（①~⑥ 完成状态）、范围划定/变更对话框（行政区任意层级/SHP，变更确认机制）、关键结论摘要卡片（冲突地块/设施盲区/耕地净减少/高度适宜占比，点击下钻对应模块）、问题识别与规划建议清单、用地结构/体检占用/适宜性/转移矩阵图表；数据来自 /api/dashboard/summary（持久化结果优先）。",
    "frontend/src/views/ParcelManagementView.vue": "地块管理（v2.0）：期次筛选（全部/基期/末期，表格与地图同步）、上传关联项目与期次、地图批量选择模式（点击地块/POI/控制线进入选中集 → 批量删除/锁定）、锁定地块（锁定后不可删除）、标注面板（地图绘制保存至 map_features 并支持锁定/删除）、详情抽屉（期次/项目/判定依据/模块跳转）。",
    "frontend/src/views/TransferMatrixView.vue": "模块一 用地变化转移矩阵（v2.0）：基期/末期 SHP 分次导入（关联项目）、一键生成演示基期、范围继承项目、矩阵透视表（色深=面积）、消失/新增图斑标签、面积增减图、变化图斑上图；模块联动按钮：对变化图斑进行合规检查（→体检）、评估新增用地适宜性（→适宜性）、分析新增用地设施可达性（→可达性，图斑并集范围随 ui store 传递）。",
    "frontend/src/views/SuitabilityView.vue": "模块二 土地适宜性评价（v2.0）：评价目标切换（建设/耕地）、因子权重滑杆（自动归一化）、范围默认继承项目（转移矩阵联动范围优先）、40×40 格网四级适宜性专题图（永久基本农田/生态保护红线刚性约束强制不适宜）、等级分布图、格网点击弹分、「查看体检结论」反向校验联动。",
    "frontend/src/views/AccessibilityView.vue": "模块三 服务设施可达性分析（v2.0）：设施类型多选、服务半径滑杆+预设、范围默认继承项目（联动范围优先）、覆盖率饼图、盲区清单（行点击定位）、覆盖/盲区红绿专题图与图斑弹窗、「推荐设施选址」（盲区∩适宜布局区，蓝色虚线展示候选区域）。",
    "frontend/src/views/PlanningCheckView.vue": "模块四 三区三线合规性批量体检（v2.0）：标准三线控制线（SHP 导入选择约束类型/图上绘制/批量删除/锁定）、规则矩阵对话框（12 地类 × 三线）、批量体检（结果持久化，台账含判定依据与重叠亩数）、变化图斑联动体检（接收转移矩阵参数自动执行）、判定依据明细表、台账 CSV 导出（含判定依据）。",
    "frontend/src/views/ReportView.vue": "报告生成（v2.0 综合分析）：继承驾驶舱分析项目与范围；六章节预览（项目概况含流程进度/现状评价/问题识别/原因分析含判定依据/规划建议/附录数据表）；问题与台账行可点击跳转对应模块/地块（钻取）；下载 Markdown。",
...
MODULE_TABLE = [
    ...
    ("数据管理", "地块管理", "/api/parcels", "分页/CRUD/视野查询/期次筛选/SHP 导入（关联项目期次）/批量删除锁定/地图标注"),
    ...
    ("模块二", "适宜性评价", "/api/analysis/suitability", "格网法多因子加权叠加 + 三区三线刚性约束 + 格网持久化 + 反向校验体检"),
...
API_TABLE = [
    ("路由前缀", "功能", "关键接口"),
    ("/api/parcels", "地块管理", "分页/GeoJSON（period 过滤）/ CRUD / import-shp（项目+期次）/ batch-delete / {id}/lock / batch-set-period"),
    ("/api/regions", "行政区划", "省/市/县列表 / GeoJSON / 详情 / 下级 / locate 定位 / import"),
    ("/api/pois", "兴趣点", "分页列表 / GeoJSON / 新建 / 删除 / batch-delete"),
    ("/api/analysis", "空间分析（模块一~三）", "transition（import-shp/generate-demo-base/matrix/patches）、suitability（targets/evaluate/grids）、accessibility（analyze/results）、facility-sites、parse-scope"),
...
def build():
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("细化到每个文件的职责说明 · v1.3")
    ...
    para(doc, "初始化顺序：00_create_database.sql（建库+扩展）→ 01_init_schema.sql（建表+索引）→ 03_regions.sql"
              "（省级行政区）→ 02_seed_data.sql（业务演示数据）；后续结构变更走 Alembic 迁移（0001 初始 → 0002 期次字段 → 0003 删除变化监测表）。", indent=0)
    ...
    para(doc, "后端测试：cd backend  →  ..\\venv\\Scripts\\python.exe -m pytest（13 项冒烟测试）", indent=14)
