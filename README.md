...
> **v4.0**：修复「网络请求失败」无法运行问题并新增顶栏后端连接状态指示；
> 地图圈选删除全面升级 —— 框选/圈选/多边形绘制范围后自动识别范围内**全部点、面要素**（地块+兴趣点+控制线）并一键批量删除（锁定项自动跳过，单次 IN 查询支撑海量数据）；
> 修复点要素不可点击、无法查看属性的 Bug（POI 点击弹属性卡片，可定位/查看详情/锁定/删除，新增 POI 锁定端点）；
> 删除转移矩阵模块重复的期次导入功能（两期数据统一在地块管理模块导入，点击计算直接出矩阵）；
> 新增期次显示开关（勾选显示哪期就显示哪期，基期实线/末期虚线描边，避免两期数据混叠杂乱）。
> **v4.0.3（稳定性增强）**：地图地块加载全面智能化 —— 缩放/平移 400ms 防抖合并请求、
> 缩小视野直接复用已加载范围（零请求）、新请求自动中止上一批在途请求（消除响应堆积）、
> 视野过大自动暂停加载（放大自动恢复）、单次返回要素封顶 2000（truncated/total 提示，圈选删除改走服务端几何匹配覆盖全量）；
> 后端连接池扩容（10+20）、限流放宽到 300 次/分钟、超时错误提示更明确，彻底解决缩放卡死与后端假死。
> **v5.0（项目化与矢量瓦片）**：项目上下文持久化，刷新后自动恢复；新建项目同时确定名称、年份与范围；
> 地块/POI/控制线/标注统一按 `project_name` 绑定当前项目；删除 `district/region_code/far_limit/height_limit` 冗余字段；
> 地块 SHP 导入字段收敛为“期次/编号/名称/用地类型”；地图切换为 PostGIS `ST_AsMVT` 矢量瓦片加载，项目全貌与局部缩放同步兼顾。
